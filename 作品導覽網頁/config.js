...
window.LangManager = { // 建立全域語系管理器物件
    ...
    setLanguage(langCode) { // 定義設定語系並更新文本的方法
        ...
        // 語系代碼防重複防護線：只有在新舊語系代碼真正不同時才執行變更，防止重複觸發排版重算
        const targetAttrValue = langCode === 'zh' ? 'zh-TW' : 'en';
        const currentAttrValue = document.documentElement.getAttribute('lang');

        if (currentAttrValue !== targetAttrValue) {
            document.documentElement.setAttribute('lang', targetAttrValue); // 執行最高根節點屬性修改
        }
