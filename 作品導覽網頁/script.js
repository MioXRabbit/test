...
/**
 * 2. 導覽列渲染（iOS 絲滑秒跳轉、防卡死修正版）
 * 負責從 config.nav 生成選單圖示、文字，並綁定 iOS 相容的點擊捲動事件
 */
function renderNav() {
    ...
    config.nav.forEach((item, i) => {
        ...
        // 💡 關鍵優化 1：移除 href 錨點，改用 javascript:void(0) 防止 iOS 預設行為干擾跳轉
        ...
        // 💡 關鍵優化 2：針對 iOS 綁定相容的 click 事件，動態計算目標位置
        navLink.addEventListener('click', (e) => {
            e.preventDefault(); // 阻斷所有預設點擊干擾

            // 尋找網頁中對應的 section 節點，例如 #sec0, #sec1
            const targetSection = document.getElementById(`sec${i}`);

            if (targetSection) {
                // 執行 iOS 完美支援的平滑捲動
                // 請確保 script.js 內的點擊事件是這樣寫：
                targetSection.scrollIntoView({
                    behavior: 'smooth', // 這裡的 smooth 會與 CSS 的吸附完美嚙合
                    block: 'start'      // 精準對齊 section 的頂端
                });
            } else {
                console.warn(`找不到目標區塊: #sec${i}，請確認 HTML 結構中的區塊 ID 是否正確。`);
            }
        });
...
function renderPodcastList() {
    ...
    // 💡 關鍵加入：每次執行渲染時，動態將 config 內的 Podcast 區塊大標題填入 HTML
    const secTitle = document.getElementById('podcastSecTitle');
    if (secTitle && window.config.podcastSectionTitle) {
        secTitle.innerText = window.config.podcastSectionTitle;
    }
