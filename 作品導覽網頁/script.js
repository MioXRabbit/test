 ...
 */
    function initNavToggleLogic() {
        ...
        // 3. 點擊選單內部的單一選項：跳轉後「自動收回」選單
        if (navList) {
            navList.addEventListener('click', (e) => {
                // 只要點擊到的是您的 nav-item 選項，就自動將選單收回隱藏
...
function renderPodcastList() {
    ...
    // 在 renderPodcastList() 函式內的開頭：
    const sectionTitle = document.getElementById('podcastSectionTitle');
    if (sectionTitle) {
        // 💡 精準抓取你設定的 config.podcastSectionTitle
        sectionTitle.textContent = config.podcastSectionTitle || "Podcast 展示";
    }
