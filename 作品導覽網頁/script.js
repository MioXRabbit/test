...
(function () {
    ...
    const WakeLockCtrl = {
        ...
        request() {
            if ('wakeLock' in navigator && !WakeLockCtrl._lockInstance) {
                // 核心時序導正：物理排除 this 指標，直接寫入 WakeLockCtrl，防止非同步時序錯位
                WakeLockCtrl._lockInstance = "pending";

                navigator.wakeLock.request('screen')
                    .then((lock) => {
                        WakeLockCtrl._lockInstance = lock; // 🌟 終極修正：物理鎖定，確保實例百分之百存入記憶體
                        lockBtn.classList.add('active'); // 點亮主題藍滿填與文字反黑
                        WakeLockCtrl.updateText(); // 0秒光速換字
        ...
        // 助手 B：手動解鎖，還原省電機能
        release() {
            // 核心時序導正：物理排除 this 指標，精準校對記憶體內真實的實例狀態
            if (WakeLockCtrl._lockInstance && WakeLockCtrl._lockInstance !== "pending") {
                WakeLockCtrl._lockInstance.release()
                    .then(() => {
                        WakeLockCtrl._lockInstance = null;
    ...
    lockBtn.onclick = () => {
        ...
        // 4. 依照當前實體高亮狀態，執行開關切換判定（徹底根除無法關閉 Bug）
        if (!lockBtn.classList.contains('active')) {
