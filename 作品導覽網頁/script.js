...
// ==========================================
// 👥 全角色「核心大立繪（m）」全自動編碼缺陷攔截與強行預存鎖（異步分流延遲版）
// ==========================================
// 🌟 關鍵優化：利用 window.onload 或是延時定時器，將大立繪下載往後排，100% 釋放網頁開局頻寬！
window.addEventListener('load', () => {
    // 💡 提示：此時網頁最核心的導覽列、播放器、CSS 已全部安全下載完畢！
    // 我們利用 setTimeout 再往後推遲 300 毫秒，等 iOS 顯示核心徹底閒下來
    setTimeout(() => {
        if (config && Array.isArray(config.chars)) {
            config.chars.forEach((char) => {
                if (char.m) {
                    ...
                    // 下載後判定：嘗試對已下載內容發動硬體加速解碼
                    imgCache.decode().then(() => {
                        console.log(`[Cache OK] [背景異步] 硬體解碼預存成功: ${char.m}`);
                    }).catch(() => {
                        // 核心防禦：瑕疵內容強行預存鎖
                        const fallbackCache = new Image();
                        fallbackCache.onload = () => {
                            console.log(`[Cache OK] [背景異步] 圖檔已強行塞進快取: ${char.m}`);
                        };
                        fallbackCache.src = char.m;
                    });
                }
            });
        }
    }, 300); // 延遲 300 毫秒放行，避開開局大塞車
});
...
document.addEventListener('play', (event) => {
    // 捕捉時序：當網頁上任何一個影片（video）或音訊（audio）被點擊並開始播放的瞬間

    const triggeredElement = event.target; // 撈出目前正在發聲的這個實體硬體節點

    // 🌟 核心攔截判定：如果目前播放的是影片，或者是一個不屬於我們 Podcast 列表的外部音軌
    if (triggeredElement.tagName === 'VIDEO' || !triggeredElement.id.startsWith('native-audio-')) {

        console.log("[Audio Core] 偵測到外來影片或音軌正在奪取播放霸權，發動強行攔截暫停");

        // 遍歷我們專案中所有的隱形音訊節點，強迫他們全部立正暫停！
        document.querySelectorAll('audio[id^="native-audio-"]').forEach((audio, idx) => {
            if (!audio.paused) {
                // 如果發現我們某個 Podcast 正在播，立刻執行實體音軌掐斷
                audio.pause();

                // 🌟 聯動修正外觀：撈出該集對應的自定義播放器按鈕外殼，同步還原為暫停狀態
                const btn = document.getElementById(`play-btn-${idx}`);
                if (btn) {
                    const iconImg = btn.querySelector('.player-icon-img');
                    const pImg = config.playIcon || "./icon/play.png";

                    if (iconImg) iconImg.src = pImg; // 換回常規播放去背圖
                    btn.classList.remove('is-playing', 'is-loading');
                    btn.classList.add('is-paused'); // 安全退回暫停外觀
                }
            }
        });
    }
}, true); // 🌟 核心關鍵：開啟 true（事件捕獲機制），強迫所有外來影片都必須先交出審查權！
    ...
    if (existingItems.length === 0) {
        ...
        navList.querySelectorAll('.nav-item').forEach((btn, i) => {
            btn.onclick = (e) => {
                ...
                // 🌟 核心新增 A：導覽列本身防連點攔截鎖
                // 檢查導覽列總容器本身此時是否正處於縮回/展開的動畫冷卻中，如果是，直接沒收點擊事件！
                if (navList.dataset.isMenuCooling === "true") return;

                // 總容器聯合鎖定：檢查總容器 navList 是否正在頁面平移冷卻中
                if (navList.dataset.isCooling === "true") return;

                // 🚀 啟動全域聯合安全鎖：將總容器標記為冷卻狀態，瞬間同步封鎖所有導覽按鈕
                navList.dataset.isCooling = "true";

                // 🌟 核心新增 B：瞬間同步發動導覽列本身的動畫冷卻鎖
                navList.dataset.isMenuCooling = "true";
                ...
                if (targetSec) {
                    ...
                    // 💡 關鍵修復：利用 iOS 異步分流鎖延時 500 毫秒，靜候 iOS 的橡皮筋 Bounce 反彈完全停穩！
                    setTimeout(() => {
                        ...
                        // 💡 關鍵修復追加：主動命令您專案中的導覽總容器將展開類別移除
                        // 🌟 這裡執行了縮回動作，導覽列會開始跑收回動畫（約 400~500ms）
                        navList.classList.remove('open', 'is-open', 'active-menu');
                    }, 500);
                }

                // ⏱️ 頁面平移安全鎖解除（原本的 500ms 保持不動）
                setTimeout(() => {
                    navList.dataset.isCooling = "false";
                }, 500);

                // 🌟 核心新增 C：導覽列本身解鎖定時器
                // 精準鎖定 500 毫秒（對齊您 CSS 導覽列收回的動畫長度），確保選單在後台完全縮回停穩之後，才允許再度點擊
                setTimeout(() => {
                    navList.dataset.isMenuCooling = "false"; // 導覽列本身完全收回，安全解除冷卻鎖
                    console.log("[Menu] 導覽列本身動畫已完全收回，防連點鎖解除");
                }, 500);
            };
        }); // 結束 forEach 遍歷監聽
...
function renderPodcastList() {
    ...
    if (titleEl) { // 安全檢查：如果標題元件存在
        titleEl.textContent = config.podcastTitle || (window.lang && lang.podcastTitle) || 'Podcast 展示'; // 透過 textContent 將大標題秒切為當前語言文本，保障語系切換時不閃爍
    } // 結束標題處理的條件區塊
            ...
            if (!existingScrollBox) { // 條件分支：如果畫面上還找不到滾動盒子結構，代表網頁是第一次載入，需要進行完整 HTML 骨架注入
                ...
                window.CustomPlayer = { // 在全域 window 物件上掛載一個名為 CustomPlayer 的獨立控制庫，用來非同步調度所有自定義播放器的圖片 src 切換與時間軸滑移
                    togglePlay(idx) { // 定義一個名為 togglePlay 的方法，用來開關特定集數的播音
                        ...
                        const lImg = config.loadingIcon || ""; // 🌟 核心補回：提撥載入中去背圖路徑
                        ...
                        if (audio.paused) { // 條件判斷：若該集目前正處於暫停或未播放狀態
                            // 排他機制：強迫其他所有集數暫停，並一對一將按鈕內部的圖片 src 全部換回常規播放去背圖檔
                            document.querySelectorAll('.player-control-btn').forEach((b, ai) => {
                                if (ai !== idx) {
                                    const aud = document.getElementById(`native-audio-${ai}`);
                                    if (aud) aud.pause();
                                    b.classList.remove('is-playing', 'is-loading'); // 排他時一併清除其他按鈕的載入狀態
                                    b.classList.add('is-paused');
                                    const img = b.querySelector('.player-icon-img');
                                    if (img) img.src = pImg;
                                }
                            });

                            // 🌟 iOS 最高防禦防線：點擊瞬間「同步」切換為轉圈圈載入中外觀，100% 避開非同步封鎖
                            if (lImg) {
                            ...
                            // 🌟 核心修正：直接、乾淨地執行播放，絕不在後方使用 .then()，彻底治癒 iOS 播音阻斷
                            audio.play().catch(() => {
