    ...
    if (existingItems.length === 0) {
        ...
        navList.querySelectorAll('.nav-item').forEach((btn, i) => {
            btn.onclick = (e) => {
                ...
                // 💡 調整重點：0秒直接切換至指定頁面，全面拔除滾動過程
                const targetSec = document.getElementById(`sec${i}`);
                if (targetSec) {
                    // 1. 強制網頁瞬間歸位到該區塊，絕不滾動
                    targetSec.scrollIntoView({ behavior: 'instant', block: 'start' });

                    // 2. 0秒直接發動入場動畫：先拔掉其他分頁的動畫類別，並為當前分頁掛上啟動動畫的 Class
                    // 備註：此處假設您的動畫啟動 Class 為 reveal，且其他區塊也用 sec 命名，若有名稱差異可再微調
