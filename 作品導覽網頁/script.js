...
(function () {
    ...
    const WakeLockCtrl = {
        ...
        _isActiveMode: false, // 🌟 終極新增：全獨立記憶體狀態鎖，不依賴 DOM 的 class，100% 杜絕外觀被重置
        ...
        updateText() {
            ...
            // 🌟 修正：完全依據記憶體內的 _isActiveMode 狀態來決定文字，徹底免疫外部 CSS/JS 洗掉類別的干擾
            if (WakeLockCtrl._isActiveMode) {
                textSpan.textContent = txtOff;
                lockBtn.classList.add('active'); // 順便強制補回外觀類別
            } else {
                textSpan.textContent = txtOn;
                lockBtn.classList.remove('active'); // 順便強制移除外觀類別
            }
        },
        ...
        request() {
            ...
            if (isLocalFile) {
                // 本地端測試環境模擬通道
                WakeLockCtrl._lockInstance = { isMock: true, release: function () { return Promise.resolve(); } };
                WakeLockCtrl._isActiveMode = true; // 鎖定狀態為開啟
                WakeLockCtrl.updateText(); // 0秒光速換字 "休眠關閉"
                console.log("[WakeLock Mock] 本地測試環境：已模擬螢幕常亮鎖定");
                return;
            }

            // --- 線上正式環境（HTTPS）---
            if ('wakeLock' in navigator && !WakeLockCtrl._lockInstance) {
                WakeLockCtrl._lockInstance = "pending";

                navigator.wakeLock.request('screen')
                    .then((lock) => {
                        WakeLockCtrl._lockInstance = lock;
                        WakeLockCtrl._isActiveMode = true; // 🌟 只有成功放行，才在記憶體中標記為開啟
                ...
                // 💡 降級防護鎖：萬一設備完全不支援 wakeLock API，利用純記憶體鎖達成外觀無縫換字切換
        ...
        // 助手 B：手動解鎖，還原省電機能
        release() {
            if (WakeLockCtrl._lockInstance) {
                const targetLock = WakeLockCtrl._lockInstance;

                if (targetLock === "pending" || targetLock.isMock) {
                    WakeLockCtrl._lockInstance = null;
                    WakeLockCtrl._isActiveMode = false; // 狀態標記為關閉
                    WakeLockCtrl.updateText();
                    return;
                }
                ...
                targetLock.release()
                    .then(() => {
                        WakeLockCtrl._lockInstance = null;
                        WakeLockCtrl._isActiveMode = false; // 狀態標記為關閉
                        WakeLockCtrl.updateText(); // 0秒光速換字 "休眠開啟"
                    })
                    .catch(() => {
                        WakeLockCtrl._lockInstance = null;
                        WakeLockCtrl._isActiveMode = false;
    ...
    lockBtn.onclick = () => {
        ...
        // 🌟 修正：完全使用獨立控制物件的布林狀態進行開關判定，100% 治好無法關閉與不換字
        if (!WakeLockCtrl._isActiveMode) {
    ...
    document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'visible') {
            // 💡 終極正確修正：醒來時，直接比對記憶體內的 _isActiveMode 旗標。
            // 只要記錄顯示先前使用者有開啟（true），管它外觀類別有沒有被洗掉，
            // 程式直接在後台補火續約，按鈕外觀與狀態再也絕對不會被意外重置彈回！
            if (WakeLockCtrl._isActiveMode) {
    ...
    // 將更新函數掛載到全域，方便您的 LangManager 在變更語系時一併呼交
