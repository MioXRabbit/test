...
/**
 * 4. Podcast 列表渲染 - 全平台相容終極相容版（⚡ 徹底根除本機死節點與 iOS 進度條漏洞）
...
function renderPodcastList() {
    ...
    if (!scrollBox) {
        ...
        // ⚡ 核心修正 1：在生成 HTML 的當下，立刻精準檢測當前是否在本機環境 (file:// 或 localhost)
        ...
        const listHtml = config.podcastList.map(item => {
            // 💡 如果在本機，直接把網址灌進 src 屬性，阻止瀏覽器沙盒判定為死節點；如果在線上，則留空交給 iOS 破壞者優化
        ...
        // ⚡ 3. 終極相容：SoundOn 轉址破壞者監聽器（僅針對線上環境產生攔截）
        container.querySelectorAll('audio').forEach(audio => {
            audio.addEventListener('play', async function handlePlayIntercept() {
                const targetUrl = this.getAttribute('data-src');

                // ⚡ 核心修正 2：如果發現目前已經有實體網址（本機環境已在第一步灌入），直接放行原生播放，完全不干涉！
                if (this.src && this.src.startsWith('http')) return;

                if (targetUrl) {
                    // ===【線上生產環境（GitHub Pages）核心防線】===
                    // 用輕量的 HEAD 請求幫 iOS 提前跑完 SoundOn 的 302 跳轉，抓出直連 MP3 網址，解鎖進度條
                    try {
                        const response = await fetch(targetUrl, { method: 'HEAD' });
                        this.src = response.url || targetUrl;
                    } catch (e) {
                        this.src = targetUrl; // 失敗時安全降級
                    }

                    // 線上環境補上載入並播放
                    this.load();
                    this.play().catch(err => console.log("Audio Play Active:", err));
                }
...
//});
       ...
       //    }
/**
 * 6. 角色展示區渲染函數 - 智慧型多語系精準替換版（⚡ 完美修正頭像高亮不跟隨 Bug）
 * 徹底解決切換語言時頭像按鈕、大立繪重新載入與閃爍的效能地雷，並保證點擊換人時發光效果精準連動
 */
function renderChar(idx) {
    ...
    if (!existingLayout) {
        ...
        if (btnWrap) {
            config.chars.forEach((char, i) => {
                ...
                // 💡 首次載入時，根據傳入的 idx 精準賦予初始的 active 類別
    ...
    // ========================================================
    // 💡 不管是【首次載入】、【手動點擊換人】還是【切換語系】，每次執行 renderChar 時
    // 我們都直接從全域 document 去抓取大容器 #char-content 內部的所有頭像按鈕
    // 這樣做 100% 絕對不會因為結構層級而抓不到按鈕，徹底點亮你的發光效果！
    const allButtons = container.querySelectorAll('.head-circle');
    if (allButtons && allButtons.length > 0) {
        allButtons.forEach((btn, i) => {
            if (i === idx) {
                btn.classList.add('active'); // 為當前選中的角色戴上發光皇冠
            } else {
                btn.classList.remove('active'); // 摘除其他未選中角色的發光
            }
        });
    }
}
