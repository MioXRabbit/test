...
/**
 * 9. 核心互動邏輯 (iOS 相容與滿版吸附修復版 + ⚡ 極致寬鬆一滑就切版)
 * 功能：整合區塊進入偵測 (IntersectionObserver)、全域捲動百分比監聽，並大幅放寬切頁滾動門檻
 */
function setupCoreLogic() {
    ...
    // --- ⚡ 關鍵新增 C：極致寬鬆的「方向意圖流」全螢幕切頁控制 ---
    let isScrollLocked = false; // 防止連續滾動造成頁面瘋狂暴衝
    let touchStartY = 0;
    const scrollCooldown = 800; // 切頁後的冷卻時間（0.8秒），配合CSS進場動畫時間
    ...
    scrollContainer.addEventListener('wheel', (e) => {
        ...
        // 💡 寬鬆門檻：只要滾動增量大於 5 像素，立刻判定為有切頁意圖！
        if (Math.abs(e.deltaY) > 5) {
            // 透過當前點亮的導覽列高亮（.active）來抓取目前在哪一頁
            const activeNav = document.querySelector('.nav-item.active');
            const currentIndex = activeNav ? navItems.indexOf(activeNav) : 0;

            if (e.deltaY > 0) {
                goToSection(currentIndex + 1); // 往下滾動 ➔ 去下一頁
            } else {
                goToSection(currentIndex - 1); // 往上滾動 ➔ 去上一頁
            }
        }
    }, { passive: false });

    // 2. 移動端 / iOS Safari 觸控滑動極速放寬
    ...
    scrollContainer.addEventListener('touchmove', (e) => {
        ...
        const diffY = touchStartY - touchEndY; // 計算手指滑動的實體距離

        // 💡 觸控寬鬆門檻：只要拇指在螢幕上抹超過 15 像素，0秒立刻判定切頁
        if (Math.abs(diffY) > 15) {
            const activeNav = document.querySelector('.nav-item.active');
            const currentIndex = activeNav ? navItems.indexOf(activeNav) : 0;

            if (diffY > 0) {
                goToSection(currentIndex + 1); // 手指向上滑 ➔ 去下一頁
            } else {
                goToSection(currentIndex - 1); // 手指向下滑 ➔ 去上一頁
            }
        }
    }, { passive: false });
