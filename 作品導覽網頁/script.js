    ...
    requestAnimationFrame(() => { // 啟動第一層動畫影格監聽：強迫 JavaScript 進入瀏覽器的排隊等待序列，確保時序分流安全
        requestAnimationFrame(() => { // 啟動第二層動畫影格監聽：強迫程式碼等待實體網頁 DOM 樹與物理像素完全繪製就緒，徹底防範空白死火
            ...
            if (!existingScrollBox) { // 條件分支：如果畫面上還找不到滾動盒子結構，代表網頁是第一次載入，需要進行完整 HTML 骨架注入
                        ...
                        if (audio.paused) { // 條件判斷：若該集目前正處於暫停或未播放狀態
                            ...
                            document.querySelectorAll('.player-control-btn').forEach((b, ai) => {
                                if (ai !== idx) {
                                    ...
                                    if (aud) aud.pause();
                                    b.classList.remove('is-playing', 'is-loading'); // 修正：排他時一併清除其他按鈕的載入狀態
                ...
                }); // 結束定時器註冊的遍歷迴圈 ─── 🌟 遍歷在此乾淨結束，絕不在迴圈內疊加多餘監聽！

                // 🚀 【核心優化】方案乙追加：網頁長掛/外來音訊切斷 醒來「滿血自我校對與斷點快取鎖」（抽離至迴圈外，100% 釋放 iOS 速度）
                document.addEventListener('visibilitychange', () => {
                    if (document.visibilityState === 'visible') {
                        // 當網頁醒來時，一次性批次安全校對所有集數，絕不堆疊重複請求
                        episodes.forEach((ep, i) => {
                            const audio = document.getElementById(`native-audio-${i}`);
                            ...
                            // 🌟 情況 A：若網頁解凍醒來時，發現原生播放器其實已經被外來通話或影片強行切斷（audio.paused 為 true），
                            // 但按鈕畫面上卻殘留著播放狀態，代表 iOS 在凍結時吃掉了原生事件！
                            if (audio.paused && (btn.classList.contains('is-playing') || btn.classList.contains('is-loading'))) {
                                if (iconImg) iconImg.src = currentPlayIcon;
                                btn.classList.remove('is-playing', 'is-loading');
                                btn.classList.add('is-paused'); // 0秒強行拉回暫停狀態，且進度時間軸（audio.currentTime）依舊完好定格在原地
                                // 🌟 這裡已精準摘除會驚動 iOS 系統而產生黑底的舊 WakeLock 釋放指令
                            }

                            // 🌟 情況 B：若網頁醒來時，原本應該是播放狀態，但 readyState < 3（代表 iOS 核心掐斷了連線通道導致失效卡死）
                            else if (!audio.paused && audio.readyState < 3) {
                                const wakeProgress = audio.currentTime; // 斷點快取：光速將被掐斷當下的進度秒數備份下來
                                audio.load(); // 底層洗牌重載，強迫 iOS 重新建立通道
                                audio.currentTime = wakeProgress; // 核心復原：時間強制還原回原地，100% 防止退回 00:00

                                // 🌟 確保音訊發聲 100% 正常（無 muted 阻斷），直接發動續播
                                audio.play().catch(() => {
