...
/**
 * 5. 渲染 Podcast 節目清單（自定義 HTML5 核心：iOS 文字變體鎖全通用版）
 */
function renderPodcastList() {
    ...
    requestAnimationFrame(() => { // 啟動第一層動畫影格監聽：強迫 JavaScript 進入瀏覽器的排隊等待序列，確保時序分流安全
        requestAnimationFrame(() => { // 啟動第二層動畫影格監聽：強迫程式碼等待實體網頁 DOM 樹與物理像素完全繪製就緒，徹底防範空白死火
            ...
            const container = document.getElementById('podcast-container'); // 從已經完全蓋好的網頁實體 DOM 當中精準選取負責掛載清單的外殼容器，並存入區域常數 container
            ...
            if (!existingScrollBox) { // 條件分支：如果畫面上還找不到滾動盒子結構，代表網頁是第一次載入，需要進行完整 HTML 骨架注入
                ...
                episodes.forEach((ep, i) => { // 直接遍歷您的實體物件陣列，i 為目前集數的索引值
                        ...
                        <div class="podcast-item">
                            ...
                            <div class="podcast-meta-info">
                                ...
                                <div class="custom-audio-player">
                                    ...
                                    <button id="play-btn-${i}" onclick="window.CustomPlayer.togglePlay(${i})" class="player-control-btn is-paused">▶&#xFE0E;</button>
                    ...
                    `; // 終極正確修正：在按鈕預設初始化標籤內部加入 &#xFE0E; 文字變體鎖，從結構層面直接封印 iOS 系統雞婆的 3D 彩色表情符號重繪
                }); // 結束單集物件陣列的遍歷迴圈
                ...
                window.CustomPlayer = { // 在全域 window 物件上掛載一個名為 CustomPlayer 的獨立控制庫，用來非同步調度所有自定義播放器的按鈕換色與時間軸滑移
                    togglePlay(idx) { // 定義一個名為 togglePlay 的方法，用來開關特定集數的播音
                        ...
                        const btn = document.getElementById(`play-btn-${idx}`); // 精準選取該集畫面上放大了 1.5 倍的實體自定義播放按鈕
                        if (!audio || !btn) return; // 安全檢查：若元件不存在則立刻退出

                        if (audio.paused) { // 條件判斷：若該集目前正處於暫停或未播放狀態
                            document.querySelectorAll('audio').forEach((aud, ai) => { if (ai !== idx) { aud.pause(); const b = document.getElementById(`play-btn-${ai}`); if (b) { b.innerHTML = '▶&#xFE0E;'; b.classList.remove('is-playing'); b.classList.add('is-paused'); } } }); // 排他機制：強迫其他正在播放的集數全部暫停，並一對一補上文字變體鎖
                            audio.play().then(() => { btn.innerHTML = '⏸&#xFE0E;'; btn.classList.remove('is-paused'); btn.classList.add('is-playing'); }).catch(() => { btn.innerHTML = '▶&#xFE0E;'; }); // 終極正確修正：成功播音後將按鈕圖案切換為帶有變體鎖的暫停符號，100% 迫使 iPhone 維持標準文字線條
                        } else { // 否則（意即該集正在放音中）
                            audio.pause(); // 觸發暫停播音
                            btn.innerHTML = '▶&#xFE0E;'; // 將按鈕圖案切換回帶有變體鎖的播放圖示
                            btn.classList.remove('is-playing'); // 觸發暫停時移出播放類別
                            btn.classList.add('is-paused'); // 觸發暫停時補上暫停類別
                        } // 結束播音開關判定
                    }, // 結束 togglePlay 方法定義
