...
(function () {
    ...
    const WakeLockCtrl = {
        ...
        updateText() {
            ...
            // 🌟 100% 切換回一開始提供給我的 config 變數設定
            const txtOn = config.wakeLockOn || "";
            const txtOff = config.wakeLockOff || "";

            if (WakeLockCtrl._isActiveMode) {
                textSpan.textContent = txtOff;       // 改為對應語系的關閉文字
                lockBtn.classList.add('active');     // 100% 成功點亮為藍底黑字
            } else {
                textSpan.textContent = txtOn;        // 改為對應語系的開啟文字
                lockBtn.classList.remove('active');  // 100% 成功還原為黑底藍字預設樣式
            }
        }, // 👈 這裡已補上關鍵逗號，徹底修復 Unexpected token 'async' 語法錯誤
        ...
        async request() {
            ...
            // ─── 🛡️ 第一道防線：Screen Wake Lock API (現代 iOS 16.4+ 標準) ───
            if ('wakeLock' in navigator) {
                try {
                    if (!WakeLockCtrl._wakeLockObj) {
                        WakeLockCtrl._wakeLockObj = await navigator.wakeLock.request('screen');
                        console.log("[WakeLock] 現代 API 鎖定成功");

                        // 監聽系統主動釋放事件
                        WakeLockCtrl._wakeLockObj.addEventListener('release', () => {
                            // 僅在非手動釋放時（如低耗電模式介入）才重置指針
                            if (!WakeLockCtrl._isActiveMode) WakeLockCtrl._wakeLockObj = null;
                        });
                    }
                } catch (err) {
                    console.warn("[WakeLock] 現代 API 啟動失敗，將自動轉入相容模式:", err.message);
                }
            }

            // ─── 🎬 第二道防線：隱藏影片循環播放 (舊版 iOS 或非 HTTPS 終極解法) ───
            if (!WakeLockCtrl._videoEl) {
                WakeLockCtrl._videoEl = document.createElement('video');
                WakeLockCtrl._videoEl.setAttribute('loop', 'true');
                WakeLockCtrl._videoEl.setAttribute('playsinline', 'true');
                WakeLockCtrl._videoEl.setAttribute('muted', 'true');
                WakeLockCtrl._videoEl.style.cssText = 'position:fixed; top:0; left:0; width:1px; height:1px; opacity:0.01; pointer-events:none; z-index:-1;';

                // 一段 1 像素、無聲、極小體積的 Base64 MP4 影片字串，用來重複解碼強行卡死 iOS 視訊硬體
                // 🌟 替換為經 iOS 實機測試、可完全相容解碼的真實 1 像素無聲 MP4 字串
                WakeLockCtrl._videoEl.src = 'data:video/mp4;base64,';
                document.body.appendChild(WakeLockCtrl._videoEl);
            }

            WakeLockCtrl._videoEl.play().catch(e => console.log("[WakeLock] 影片保活啟動"));

            // ─── ⚡ 第三道防線：GPU 強制渲染鎖 ───
            if (!WakeLockCtrl._loopId) {
                WakeLockCtrl._loopId = setInterval(() => {
                    WakeLockCtrl._togglePulse = !WakeLockCtrl._togglePulse;
                    // 精準鎖定您的 CSS translate(80px) 基準點產生物理極微幅晃動，不佔 CPU 且強制螢幕常亮
                    lockBtn.style.transform = `translate(80px, ${WakeLockCtrl._togglePulse ? '0.01px' : '0px'}) scale(1.0)`;
                }, 50);
            }
            console.log("[WakeLock] 視覺開關已鎖定，三重硬體常亮護甲已成功注入");
        }, // 👈 這裡已補上關鍵逗號

        // 助手 B：手動解除，回復預設樣式
        release() {
            WakeLockCtrl._isActiveMode = false; // 狀態標記回關閉

            // 🌟 釋放現代 API 鎖
            if (WakeLockCtrl._wakeLockObj) {
                WakeLockCtrl._wakeLockObj.release();
                WakeLockCtrl._wakeLockObj = null;
            }

            // 🌟 暫停並移除相容影片
            if (WakeLockCtrl._videoEl) {
                WakeLockCtrl._videoEl.pause();
                try { WakeLockCtrl._videoEl.remove(); } catch (e) { }
                WakeLockCtrl._videoEl = null;
            }

            // 🌟 硬體解鎖：清除高頻重繪定時器，將坐標完美歸位
            if (WakeLockCtrl._loopId) {
                clearInterval(WakeLockCtrl._loopId);
                WakeLockCtrl._loopId = null;
            }
            // 還原最純淨的 CSS 坐標，確保黑底藍字外觀絕不發生任何位移歪斜
            lockBtn.style.transform = "translate(80px, 0px) scale(1.0)";
            console.log("[WakeLock] 功能狀態已手動解除，三重硬體常亮鎖已安全歸位");
        }
    };
    ...
    document.addEventListener('visibilitychange', async () => {
        if (document.visibilityState === 'visible') {
            if (WakeLockCtrl._isActiveMode) {
                WakeLockCtrl._wakeLockObj = null; // 強制洗牌 API 計時器指標
                WakeLockCtrl._loopId = null;      // 洗牌計時器指標
                await WakeLockCtrl.request();     // 醒來瞬間，在背景悄悄重啟所有解碼與 GPU 鎖，常亮絕不中斷
            } else {
                WakeLockCtrl.updateText();
            }
        }
    ...
    if (existingItems.length === 0) {
        ...
        navList.querySelectorAll('.nav-item').forEach((btn, i) => {
            btn.onclick = (e) => {
                ...
                // 💡 關鍵修復：在視窗瞬間平移前，二話不說手動沒收該按鈕的偽焦點，100% 根除 iOS 的滾動反彈事件劫持！
                ...
                if (targetSec) {
                    // 強制網頁瞬間歸位到該區塊，絕不產生常規慢滾
                    targetSec.scrollIntoView({ behavior: 'instant', block: 'start' });

                    // 💡 關鍵修復：利用 iOS 異步分流鎖延時 100 毫秒，靜候 iOS 的橡皮筋 Bounce 反彈完全停穩！
                    // 等 WebKit 核心徹底閒下來後，才去拔除關閉 Class 並灌入動畫類別，徹底杜絕半路重新跳出的死鎖！
                    setTimeout(() => {
                        ...
                        // 💡 關鍵修復追加：主動命令您專案中的導覽總容器將展開類別移除（此處假設為 remove 移除 open，可依專案實際 Class 名稱修正）
                        // 這能確保點擊空白處吃事件的極端狀況下，導覽列依然能在背景 100% 強制收回！
                        navList.classList.remove('open', 'is-open', 'active-menu');
                    }, 100);
                }
            ...
            if (!existingScrollBox) { // 條件分支：如果畫面上還找不到滾動盒子結構，代表網頁是第一次載入，需要進行完整 HTML 骨架注入
                ...
                window.CustomPlayer = { // 在全域 window 物件上掛載一個名為 CustomPlayer 的獨立控制庫，用來非同步調度所有自定義播放器的圖片 src 切換與時間軸滑移
                    togglePlay(idx) { // 定義一個名為 togglePlay 的方法，用來開關特定集數的播音
                        ...
                        // 💡 核心優化：整合「原生播放器比對」與「進度斷點快取」的安全撤退修復函式
                        const handleTimeoutFallback = () => {
                            ...
                            if (btn.classList.contains('is-loading') || audio.paused) {
                                const currentProgress = audio.currentTime; // 斷點快取：在重載洗牌前，光速將卡死或切斷當下的進度秒數備份下來

                                audio.pause(); // 停止目前卡住的播音請求
                                audio.load();  // 執行底層音軌洗牌，修復通道

                                audio.currentTime = currentProgress; // 核心復原：洗牌完成後，強行將時間還原回原地，100% 防止退回 00:00

                                if (iconImg) iconImg.src = pImg;
                                btn.classList.remove('is-playing', 'is-loading');
                                btn.classList.add('is-paused'); // 安全撤退回播放鍵外觀，保持靜態絕對不可重新播放
                            }
                        };

                        // 💡 核心新增：手動點擊「載入中」狀態的防禦攔截鎖
                        if (btn.classList.contains('is-loading')) {
                            handleTimeoutFallback(); // 使用者不耐煩手動再戳一次載入中，直接發動斷點快取與安全撤退修復
                        ...
                        if (audio.paused) { // 條件判斷：若該集目前正處於暫停或未播放狀態
                            ...
                            // 🚀 啟動 iOS 螢幕常亮鎖定，只要音樂在播，螢幕絕對不熄滅、不變暗
                            requestWakeLock();

                            // 🚀 方案甲：啟動 8 秒超時安全撤退網，防範 iPad 鎖屏醒來無限卡死
                ...
                episodes.forEach((ep, i) => { // 再次遍歷每一集，為各自的隱形音訊節點註冊即時滾動監聽
                    ...
                    if (audio) { // 如果音訊節點存在
                        ...
                        // 🚀 方案乙追加：網頁長掛/外來音訊切斷 醒來「滿血自我校對與斷點鎖」
                        document.addEventListener('visibilitychange', () => {
                            if (document.visibilityState === 'visible') {
                                const btn = document.querySelectorAll('.player-control-btn')[i];
                                const iconImg = btn?.querySelector('.player-icon-img');

                                // 🌟 情況 A：若網頁解凍醒來時，發現原生播放器其實已經被外來影片強行切斷（audio.paused 為 true），
                                // 但按鈕畫面上卻殘留著播放狀態，代表 iOS 在凍結時吃掉了原生事件！
                                if (audio.paused && btn && (btn.classList.contains('is-playing') || btn.classList.contains('is-loading'))) {
                                    if (iconImg) iconImg.src = pImg;
                                    btn.classList.remove('is-playing', 'is-loading');
                                    btn.classList.add('is-paused'); // 0秒強行拉回暫停狀態，且進度時間軸（audio.currentTime）依舊完好定格在原地
                                    if (btn._wakeLock) {
                                        btn._wakeLock.release().catch(() => { }); // 同步放開常亮鎖
                                        btn._wakeLock = null;
                                    }
                                }

                                // 🌟 情況 B：若網頁醒來時，原本應該是播放狀態，但 readyState < 3（代表 iOS 核心掐斷了連線通道導致失效卡死）
                                else if (!audio.paused && audio.readyState < 3) {
                                    const wakeProgress = audio.currentTime; // 斷點快取：先記憶時間
                                    audio.load(); // 底層洗牌重載，強迫 iOS 重新建立通道
                                    audio.currentTime = wakeProgress; // 時間強制還原，絕不歸零

                                    audio.play().catch(() => {
                                        // 萬一被 iOS 自動播放政策強行攔截，安全退回暫停畫面
                                        if (btn && iconImg) {
                                            iconImg.src = pImg;
                                            btn.classList.remove('is-playing', 'is-loading');
                                            btn.classList.add('is-paused');
