...
/**
 * 5. 渲染 Podcast 節目清單（自定義控制項：播放/暫停鍵獨立類別分家版）
 */
        ...
        requestAnimationFrame(() => { // 啟動第二層動畫影格監聽：強迫程式碼等待實體網頁 DOM 樹與物理像素完全繪製就緒，徹底防範空白死火
            ...
            if (!existingScrollBox) { // 條件分支：如果畫面上還找不到滾動盒子結構，代表網頁是第一次載入，需要進行完整 HTML 骨架注入
                ...
                episodes.forEach((ep, i) => { // 直接遍歷您的實體物件陣列，i 為目前集數的索引值
                        ...
                        <div class="podcast-item">
                            ...
                            <div class="podcast-meta-info">
                                ...
                                <div class="custom-audio-player">
                                    ...
                                    <button id="play-btn-${i}" onclick="window.CustomPlayer.togglePlay(${i})" class="player-control-btn is-paused">▶</button>
                    ...
                    `; // 預設初始為暫停狀態：在 button 標籤上掛載基礎類別名稱 is-paused，解鎖 CSS 獨立控制大門
                }); // 結束單集物件陣列的遍歷迴圈
                ...
                window.CustomPlayer = { // 在全域 window 物件上掛載一個名為 CustomPlayer 的獨立控制庫，用來非同步調度所有自定義播放器的按鈕換色與時間軸滑移
                    togglePlay(idx) { // 定義一個名為 togglePlay 的方法，用來開關特定集數的播音
                        ...
                        if (audio.paused) { // 條件判斷：若該集目前正處於暫停或未播放狀態
                            document.querySelectorAll('audio').forEach((aud, ai) => { if (ai !== idx) { aud.pause(); const b = document.getElementById(`play-btn-${ai}`); if (b) { b.textContent = '▶'; b.classList.remove('is-playing'); b.classList.add('is-paused'); } } }); // 排他機制：強迫其他正在播放的集數全部暫停，並將按鈕集體換回播放圖案、同步重置類別名稱
                            audio.play().then(() => { btn.textContent = '⏸'; btn.classList.remove('is-paused'); btn.classList.add('is-playing'); }).catch(() => { btn.textContent = '▶'; }); // 終極相容：成功播放後動態切換類別名稱為 is-playing，將高階排版控制權交棒給外部樣式表
                        } else { // 否則（意即該集正在放音中）
                            audio.pause(); // 觸發暫停播音
                            btn.textContent = '▶'; // 將按鈕圖案切換回播放圖示
                            btn.classList.remove('is-playing'); // 觸發暫停時摘除播放狀態類別名稱
                            btn.classList.add('is-paused'); // 觸發暫停時無縫補上暫停狀態類別名稱
                        } // 結束播音開關判定
                    }, // 結束 togglePlay 方法定義
