...
// 閉合 document.addEventListener('DOMContentLoaded', ...) 的事件回呼函式與小括號，網頁首次載入的初始化流程在此全部收尾

// 💡 監聽 2：當 LangManager 觸發語系改變時，0 秒立刻重新渲染所有文字（iOS 絲滑秒切關鍵）
// 以上為單行註解：說明註冊第二個全域監聽器，用來承接 LangManager 發出的事件，達到無縫切換多國語言的效果

window.addEventListener('languageChanged', () => {
    // 在全域 window 物件上註冊對自訂事件 'languageChanged' 的監聽

    runAllRenderModules();
    // 當接收到語系改變的通知時，立刻呼叫中央渲染調度器，0 秒內將全網頁的 HTML 文本與影片標題全部動態重刷一遍
});
// 結束 languageChanged 事件監聽器的定義與閉合
 ...
 */
// 以上為區塊註解：說明此函式為第四個渲染模組「Podcast 列表渲染」，其核心特色是與 SoundOn 的 RSS 非同步資料相容，並解決了 iOS 移動端上的原生播放器相容性（進度條、跨域放音）痛點

function renderPodcastList() {
    // 定義一個名為 renderPodcastList 的函式，用來動態產生或更新網頁中的 Podcast 單集清單與播放器結構

    const container = document.getElementById('podcast-container');
    // 從網頁 DOM 當中選取 ID 為 'podcast-container' 的 Podcast 大容器元素，並存入區域常數 container 當中

    // 💡 保持原有防禦：確保您從 SoundOn RSS 預載並動態生成的 config.podcastList 資料已經存在
    // 單行註解：說明下方的安全檢查機制，是用來避免在 RSS 資料尚未非同步加載完成前就提早觸發渲染導致死鎖

    if (!container || !config.podcastList || config.podcastList.length === 0) return;
    // 安全檢查：若網頁中找不到容器、或者全域設定檔中的 podcastList 清單不存在或內含元素數量為零，則立刻中斷執行

    // 1. 精準抓取總標題
    // 單行註解：說明第 1 步負責處理該區塊大標題的文字替換

    const sectionTitleEl = document.getElementById('podcastSectionTitle');
    // 從網頁中選取 ID 為 'podcastSectionTitle' 的標題元素，並存入區域常數 sectionTitleEl

    if (sectionTitleEl && config.podcastSectionTitle) {
        sectionTitleEl.textContent = config.podcastSectionTitle;
    }
    // 如果標題元素存在且全域設定檔中有該區塊的標題資料，則使用 textContent 更新其文字內容

    // 2. 智慧型多語系判定：檢查畫面上是否「已經有」產生好的 Podcast 滾動清單了？
    // 單行註解：說明下方的條件分支判斷是用來區分「初次建立播放清單」與「點擊切換語言」兩種不同情境

    const scrollBox = container.querySelector('.podcast-scroll-box');
    // 在 container 內部搜尋是否已經有名為 '.podcast-scroll-box' 的滾動外殼元素，並存入常數 scrollBox

    if (!scrollBox) {
        // 條件判斷：如果畫面上還找不到 scrollBox 元素，代表網頁是第一次載入（或 RSS 首次成功獲取資料）

        // ===【首次載入網頁】===
        // 當您的 RSS 腳本在背景解析完畢、第一次驅動本函數時，我們用最標準的 HTML5 標準結構畫出清單
        // 以上兩行為單行註解：說明首次渲染時，系統會動態編織出一套標準的 HTML5 播放器列表結構
        ...
        `).join('');
        // 讀取 config.podcastList 並使用 map 方法，將每一集資料（包含封面圖路徑、單集標題、原生控制項、預載元資料參數與音訊 MP3 網址）包裝進各自的樣板字串中，最後用 join('') 連接成一串完整的 HTML 字串並指派給常數 listHtml

        // 將最乾淨、最标准的結構注入大框架中
        // 單行註解：說明將編織好的字串正式寫入網頁結構

        container.innerHTML = `<div class="podcast-scroll-box" style="width: 100%; box-sizing: border-box;">${listHtml}</div>`;
        // 外層包裝一個寬度自適應的滾動外殼 div，透過 innerHTML 一次性注入到 container 容器中

        // ❌ 這裡完全沒有任何 audio.addEventListener('play') 的大干涉程式碼！
        // 讓播放器以最純粹的原生姿態去跑，完全不打擾您原本精心調度好的 RSS 非同步載入機制。
        // 以上兩行為單行註解：技術備忘錄，強調不使用任何主動攔截放音的監聽器，保持瀏覽器對原生音訊元件的最高相容性

    } else {
        // 否則（意即 scrollBox 已經存在，代表導覽列/列表骨架早已產生，此時使用者是在切換語言）

        // ===【非首次載入（使用者點擊切換語言時）===
        // ⚡ 結構鎖定核心：您的大框架、所有從 RSS 撈出來的封面圖、播放器節點通通留在原位！
        // 使用者一邊播著 Podcast、一邊在最上面切換「繁中 / EN」，音樂不停播、進度不重置，我們只精準用 textContent 去更新 h4 的中英文單集標題文字
        // 以上三行為單行註解：詳細說明在語系切換時採取的「音樂不斷播、進度不重置」的結構鎖定優化哲學

        const titleElements = scrollBox.querySelectorAll('.podcast-episode-title');
        // 在已經生成的滾動盒子內，精準搜出所有 class 為 '.podcast-episode-title' 的單集標題 <h4> 節點，並存入常數 titleElements

        config.podcastList.forEach((item, index) => {
            // 使用 forEach 迴圈遍歷最新語系狀態下的 config.podcastList 陣列資料

            if (titleElements[index]) {
                titleElements[index].textContent = item.title;
            }
            // 安全檢查：若對應索引值的單集標題節點存在，則直接用 textContent 修改它的純文字內容
        });
        // 結束 podcastList 陣列的遍歷更新
    }
    // 結束首次與非首次載入的條件分支判定

    // 確保容器高度自適應
    // 單行註解：說明最後的高度修正，防止內容被高度限制而截斷

    container.style.height = "auto";
    // 將大容器的 CSS 高度（height）強制設定為 "auto"，確保在單集列表展開後能流暢地撐開父層排版
}
// 結束 renderPodcastList 函式的定義
    ...
    if (!existingLayout) {
        ...
        if (btnWrap) {
            config.chars.forEach((char, i) => {
                ...
                btn.className = 'head-circle'; // 終極正確微調：首次生成時在虛擬圖層中只給予基礎名稱，不提前夾帶 active，防止引發 iOS 渲染陰影與時序錯位
                btn.onclick = () => renderChar(i); // 為這個頭像按鈕綁定原生的點擊事件監聽器，當使用者點擊該頭像時，會非同步再次呼叫 renderChar 函式
                btn.innerHTML = `<img src="${char.h}" alt="head">`; // 使用 innerHTML 在按鈕內部灌入一張圖片標籤，其圖片路徑為該角色的小頭像
                btnWrap.appendChild(btn); // 將這個建立、設定完畢的頭像按鈕元素實體，動態掛載到按鈕容器 btnWrap 的末尾
            });
        }
