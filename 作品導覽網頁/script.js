...
/**
 * 10. 全站所有頁面通用：全域防觸控雙擊放大攔截系統
 */
document.addEventListener("touchend", (event) => { // 監聽全網頁所有分頁最上層的觸控結束事件
    const currentTime = Date.now(); // 取得當前使用者手指離開螢幕的精確時間戳記
    const previousTime = window.lastGlobalTouchTime || 0; // 提取暫存在全域環境中的上一次點擊時間

    if (currentTime - previousTime <= 300) { // 判定兩次觸控結束的時間差在 300 毫秒以內（即標準雙擊手勢）
        event.preventDefault(); // 【核心防護】強制直接阻斷瀏覽器預設的雙擊全網頁放大縮放行為
    }

    window.lastGlobalTouchTime = currentTime; // 將當前的時間戳記覆寫回全域變數中供下一次點擊比對
}, { passive: false }); // 必須強制關閉被動監聽，否則 preventDefault 攔截預設行為將會失效
