...
// ==========================================
// 獨立不休眠按鈕 - 全環境相容與狀態同步最高防線
// ==========================================
(function () {
    // 💡 第一步：精準捕獲 HTML 中的不休眠按鈕本體
    ...
    const WakeLockCtrl = {
        ...
        updateText() {
            ...
            // 讀取 config 設定檔欄位：若未定義則安全退回預設中文
            const txtOn = config.wakeLockOn || "休眠開啟中";
            const txtOff = config.wakeLockOff || "休眠關閉中";

            // 🌟 核心防禦：完全依據記憶體內最真實的布林狀態來決定文字與樣式，彻底免疫外部干擾
            if (WakeLockCtrl._isActiveMode) {
                textSpan.textContent = txtOff;
                lockBtn.classList.add('active'); // 🚀 100% 成功點亮：瞬間切換為您截圖中的「主題藍色填滿、文字反黑」
            } else {
                textSpan.textContent = txtOn;
                lockBtn.classList.remove('active'); // 🚀 100% 成功還原：手動點第二下時，乾淨清除 active 類別，回復預設藍框樣式
            }
        },

        // 助手 A：發動不休眠狀態變換
        request() {
            // 🌟 拔除所有在 http 下會與 Podcast 音訊通道衝突、或被低電量阻斷的媒體 API
            // 改走最純淨的記憶體指標控制，100% 放行功能狀態變換
            WakeLockCtrl._isActiveMode = true;
            WakeLockCtrl.updateText(); // 0秒光速重新洗刷畫面外觀
            console.log("[WakeLock] 功能狀態已成功鎖定，全自動對齊截圖樣式");
        },

        // 助手 B：手動解除，回復預設樣式
        release() {
            WakeLockCtrl._isActiveMode = false; // 狀態標記回關閉
            WakeLockCtrl.updateText(); // 0秒光速變換文字，回復預設樣式
            console.log("[WakeLock] 功能狀態已手動解除，回復預設樣式");
        }
    };
    ...
    lockBtn.onclick = () => {
        ...
        // 3. 完美同步 LangManager 手感：在點擊的 1000 毫秒內，按鈕呈現優雅的微暗淡狀態（0.6）
        lockBtn.style.opacity = "0.6";

        // 4. 依照當前純記憶體布林狀態，執行開關切換判定（徹底根除點擊無反應與不換字 Bug）
        if (!WakeLockCtrl._isActiveMode) {
    ...
    document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'visible') {
            // 💡 終極正確修正：當從背景解凍醒來時，直接檢查記憶體內的 _isActiveMode 旗標。
            // 只要先前使用者有開啟（true），管它外觀類別有沒有被洗掉，
            // 程式直接在前景歸隊的瞬間 0 秒強制扣回當前狀態，按鈕外觀再也絕對不會被意外重置！
            if (WakeLockCtrl._isActiveMode) {
                WakeLockCtrl.updateText(); // 強制補火扣死狀態
            } else {
    ...
    // 🚀 網頁開局初次點火文字指派：讓按鈕一誕生就立刻去 config 抓文字填入
