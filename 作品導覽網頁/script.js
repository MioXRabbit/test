...
(function () {
    ...
    // 🌟 修正：開局「只」進行外觀類別清洗，絕對不碰任何硬體 API 或影片變數
    if (textSpan) {
        textSpan.textContent = config.wakeLockOn || "";
    }
    lockBtn.classList.remove('active');
    ...
    window.updateWakeLockBtnLang = () => {
        // 只有在外部語系切換時，才安全改寫文字，不干擾硬體狀態
        if (textSpan) {
            textSpan.textContent = WakeLockCtrl._isActiveMode ? (config.wakeLockOff || "") : (config.wakeLockOn || "");
        }
    };
