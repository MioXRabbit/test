...
(function () {
    ...
    const WakeLockCtrl = {
        ...
        async request() {
            ...
            // 在 request() 觸發時建立系統級音訊防線
            if (!WakeLockCtrl._audioEl) {
                WakeLockCtrl._audioEl = new Audio();
                WakeLockCtrl._audioEl.loop = true; // 開啟無限循環

                // 一段通過 iOS 認證、完全無聲、體積極小的 0.1 秒真實 MP3 Base64 數據鏈
                WakeLockCtrl._audioEl.src = 'data:audio/mpeg;base64,SUQzBAAAAAAAI1RTU0UAAAAPAAADTGFtZTMuOTkuNVVVVVVVVVVVVVVVVVVVVVVVVf/M0MUAAAA00HAwAAIdVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVV';
            }
            // 強制在使用者點擊的當下，同步發動音訊解碼
            WakeLockCtrl._audioEl.play().catch(e => console.log("音訊保活阻斷啟動"));
        ...
        release() {
            ...
            if (WakeLockCtrl._audioEl) {
                WakeLockCtrl._audioEl.pause();
                WakeLockCtrl._audioEl = null;
            }
            ...
            if (!existingScrollBox) { // 條件分支：如果畫面上還找不到滾動盒子結構，代表網頁是第一次載入，需要進行完整 HTML 骨架注入
                ...
                window.CustomPlayer = { // 在全域 window 物件上掛載一個名為 CustomPlayer 的獨立控制庫，用來非同步調度所有自定義播放器的圖片 src 切換與時間軸滑移
                    togglePlay(idx) { // 定義一個名為 togglePlay 的方法，用來開關特定集數的播音
                        ...
                        // 💡 核心修正：釋放 iOS 螢幕常亮鎖助手（純音軌回收，不碰座標，完美隱藏黑底）
                        const releaseWakeLock = () => {
                            // 乾淨俐落清除無聲短音定時器，釋放網頁內部硬體
                            if (btn._audioTimerId) {
                                clearInterval(btn._audioTimerId);
                                btn._audioTimerId = null;
                            }
                            if (btn._shadowAudio) {
                                btn._shadowAudio.pause();
                                btn._shadowAudio = null;
                            }
                            console.log("[WakeLock] 背景保活音軌已手動釋放");
                        };

                        // 💡 核心修正：向 iOS 申請螢幕常亮鎖助手（純網頁間歇短音，不碰座標，100% 屏蔽黑底）
                        const requestWakeLock = () => {
                            // 建立全隱藏、非串流的背景無聲短音物件
                            if (!btn._shadowAudio) {
                                btn._shadowAudio = new Audio();
                                btn._shadowAudio.loop = false; // 🌟 核心關鍵：嚴禁開啟 loop，這是屏蔽黑底控制器的死命令！
                                btn._shadowAudio.src = 'data:audio/mpeg;base64,SUQzBAAAAAAAI1RTU0UAAAAPAAADTGFtZTMuOTkuNVVVVVVVVVVVVVVVVVVVVVVVVf/M0MUAAAA00HAwAAIdVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVV';
                            }

                            // 在大拇指觸碰按鈕的當下先偷放第一次，解鎖 iOS WebKit 權限沙盒
                            btn._shadowAudio.play().catch(() => { });

                            // 每 8 秒在背景悄悄重播一次 0.1 秒的點擊短音，用非串流機制硬咬住 iOS 顯示晶片不休眠
                            if (!btn._audioTimerId) {
                                btn._audioTimerId = setInterval(() => {
                                    if (btn._shadowAudio) {
                                        btn._shadowAudio.currentTime = 0; // 進度光速歸零
                                        btn._shadowAudio.play().catch(() => { }); // 觸發無感保活
                                    }
                                }, 8000);
                            }
                            console.log("[WakeLock] 已啟動背景非串流保活，成功繞過 iOS 系統黑底機制");
                        };
