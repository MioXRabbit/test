            ...
            if (!existingScrollBox) { // 條件分支：如果畫面上還找不到滾動盒子結構，代表網頁是第一次載入，需要進行完整 HTML 骨架注入
                ...
                window.CustomPlayer = { // 在全域 window 物件上掛載一個名為 CustomPlayer 的獨立控制庫，用來非同步調度所有自定義播放器的圖片 src 切換與時間軸滑移
                    togglePlay(idx) { // 定義一個名為 togglePlay 的方法，用來開關特定集數的播音
                        ...
                        const btn = document.getElementById(`play-btn-${idx}`); // 精準選取該集畫面上放大了 1.5倍的實體自定義播放按鈕外殼
                        if (!audio || !btn) return; // 安全檢查：若元件不存在則立刻退出

                        const iconImg = btn.querySelector('.player-icon-img'); // 在當前被點擊的按鈕內部撈出負責顯示去背圖檔的 img 標籤實體
                        const pImg = config.playIcon || ""; // 提撥播放去背圖路徑
                        const sImg = config.pauseIcon || ""; // 提撥暫停去背圖路徑
                ...
                episodes.forEach((ep, i) => { // 再次遍歷每一集，為各自的隱形音訊節點註冊即時滾動監聽
                    ...
                    if (audio) { // 如果音訊節點存在
                        const fmt = (s) => { if (isNaN(s)) return '00:00'; const m = Math.floor(s / 60); const sec = Math.floor(s % 60); return `${m < 10 ? '0' : ''}${m}:${sec < 10 ? '0' : ''}${sec}`; }; // 建立一個名為 fmt 的區域時間格式化小助手

                        audio.addEventListener('timeupdate', () => { // 即時數據指派
                            if (timeTxt) timeTxt.textContent = `${fmt(audio.currentTime)} / ${fmt(audio.duration)}`;
                            if (timeline && audio.duration) {
                                const pct = (audio.currentTime / audio.duration) * 100;
                                timeline.value = pct;
                                timeline.style.setProperty('--value', `${pct}%`);
                            }
                        });

                        audio.addEventListener('loadedmetadata', () => { // 註冊元資料監聽
                            if (timeTxt) timeTxt.textContent = `00:00 / ${fmt(audio.duration)}`;
                        });
                        ...
                        document.addEventListener('visibilitychange', () => {
                            if (document.visibilityState === 'visible') {
                                ...
                                // 🌟 核心修正：即時從 config 抓取播放去背圖路徑，徹底根治 pImg is not defined 的 ReferenceError 錯誤
                                const currentPlayIcon = config.playIcon || "./icon/play.png";

                                // 🌟 情況 A：若網頁解凍醒來時，發現原生播放器其實已經被外來影片強行切斷
                                if (audio.paused && btn && (btn.classList.contains('is-playing') || btn.classList.contains('is-loading'))) {
                                    if (iconImg) iconImg.src = currentPlayIcon; // 使用修正後的圖檔變數
                                    btn.classList.remove('is-playing', 'is-loading');
                                    btn.classList.add('is-paused'); // 0秒強行拉回暫停狀態

                                    // 🌟 這裡已精準摘除原本驚動 iOS 的 btn._wakeLock 系統解鎖指令
                                }

                                // 🌟 情況 B：若網頁醒來時，原本應該是播放狀態，但 readyState < 3
                                    ...
                                    audio.play().catch(() => {
                                        ...
                                        if (btn && iconImg) {
                                            iconImg.src = currentPlayIcon; // 使用修正後的圖檔變數
                                            btn.classList.remove('is-playing', 'is-loading');
                                            btn.classList.add('is-paused');
                                        }
                                    });
                ...
                episodes.forEach((ep, i) => { // 遍歷最新的多國語言單集資料
                    ...
                    if (titleEl) { // 如果該集標題存在
                        titleEl.textContent = ep.title || ''; // 直接使用 textContent 替換標題為最新語系的 ep.title 文字
                    }
                });
            }
        });
