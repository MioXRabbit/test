            ...
            if (!existingScrollBox) { // 條件分支：如果畫面上還找不到滾動盒子結構，代表網頁是第一次載入，需要進行完整 HTML 骨架注入
                ...
                episodes.forEach((ep, i) => { // 再次遍歷每一集，為各自的隱形音訊節點註冊即時滾動監聽
                    ...
                    if (audio) { // 如果音訊節點存在
                        ...
                        // 💡 封裝一個專屬的「即時渲染更新小助手」，用來在 timeupdate 與醒來時共用
                        const updateVisuals = () => {
                        ...
                        // 🚀 核心追加：網頁長掛/鎖屏醒來自我校對防線
                        // 當使用者切換分頁、或者手機熄滅螢幕後重新點亮，網頁重新回到前景的瞬間觸發
                        document.addEventListener('visibilitychange', () => {
                            if (document.visibilityState === 'visible' && !audio.paused) {
                                // 🌟 檢查關鍵：如果目前網頁已經可見，且該集音訊正在播放中，強制跳過 timeupdate 的等待，立即手動刷新進度，100% 復活進度條
                                updateVisuals();
                            }
                        });
                    }
