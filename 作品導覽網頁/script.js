        ...
        requestAnimationFrame(() => { // 啟動第二層動畫影格監聽：強迫程式碼等待實體網頁 DOM 樹與物理像素完全繪製就緒，徹底防範空白死火
            ...
            if (!existingScrollBox) { // 條件分支：如果畫面上還找不到滾動盒子結構，代表網頁是第一次載入，需要進行完整 HTML 骨架注入
                ...
                episodes.forEach((ep, i) => { // 再次遍歷每一集，為各自的隱形音訊節點註冊即時滾動監聽
                    ...
                    if (audio) { // 如果音訊節點存在
                        const fmt = (s) => { if (isNaN(s)) return '00:00'; const m = Math.floor(s / 60); const sec = Math.floor(s % 60); return `${m < 10 ? '0' : ''}${m}:${sec < 10 ? '0' : ''}${sec}`; }; // 時間格式化助手
                ...
                }); // 結束 episodes 的遍歷監聽註冊 ─── 🌟 迴圈在這裡乾淨結束！

                // 🚀 【完美優化】將方案乙抽離到迴圈外，全網頁只維持「單一最高指揮監聽」，100% 釋放 iOS 緩衝區堵塞
