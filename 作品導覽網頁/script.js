...
/**
 * 4. Podcast 列表渲染 - 全平台相容終極版
 * 解決本機 CORS 被阻斷無法播放、以及 iOS Safari 轉址罷工的雙重地雷
 */
function renderPodcastList() {
    ...
    // 2. 生成每一集的 HTML
    // ⚡ 關鍵修正 1：徹底移除 crossorigin="anonymous"，確保你在本機測試、雙擊 HTML 時不會因為 CORS 政策被瀏覽器阻斷
    // ⚡ 關鍵修正 2：網址依然直上 audio src 屬性，並加上 preload="metadata"，保持 iOS 的基本轉址通訊能力
    ...
    // 3. 注入內容到專屬大框架中
    container.innerHTML = `
            <div class="podcast-scroll-box" style="width: 100%; box-sizing: border-box;">
                ${listHtml}
            </div>
            `;

    // 4. ⚡ 雙重保險動態攔截器：同時搞定本機與 iOS
    // 當使用者用任何裝置（特別是 iOS）按下原生播放按鈕的那一瞬間，由 JavaScript 介入「同步預熱」
    container.querySelectorAll('audio').forEach(audio => {
        audio.addEventListener('play', function handlePlayIntercept() {
            // 如果遇到 iOS 判定媒體未就緒（readyState === 0）或因為轉址產生的微小快取暫停
            if (this.readyState === 0) {
                // 強制驅動一次同步加載，這在 iOS 觸控事件的上下文裡是合法且被允許跟隨 302 轉址的
                this.load();
                this.play().catch(err => console.log("Bypass play error:", err));
            }
            // 執行一次後即自行銷毀監聽，不打擾後續的日常播放與暫停控制
