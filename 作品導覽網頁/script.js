...
// ==========================================
// 獨立不休眠按鈕 - 全環境相容與硬體常亮終極完結防線
// ==========================================
(function () {
    // 1. 精準捕獲 HTML 中的不休眠按鈕本體
    const lockBtn = document.getElementById('wakeLockBtn');
    if (!lockBtn) return;

    const textSpan = lockBtn.querySelector('.wakelock-text');

    // 🚀 物理保活黑科技：動態在網頁邊界外建立 1x1 像素全透明無聲影片，0 網路開銷，iOS 會無條件放行常亮特權
    let wakeVideo = document.getElementById('iosWakeVideo');
    if (!wakeVideo) {
        wakeVideo = document.createElement('video');
        wakeVideo.id = 'iosWakeVideo';
        wakeVideo.loop = true;
        wakeVideo.muted = true;
        wakeVideo.setAttribute('playsinline', '');
        wakeVideo.style.cssText = "position:absolute; width:1px; height:1px; opacity:0; pointer-events:none; top:-99px;";
        document.body.appendChild(wakeVideo);
    }

    // 超微型 1 秒全黑無聲影片的 Base64 數據，不佔用任何顯示晶片（GPU）算力
    const dummyVideoSrc = "data:video/mp4;base64, bHN0AAAAJi0tLS0AAAAhaG1kaAAAAABkYXRhAAAAAQAAAAEAAAAAbXA0Mg==";
    ...
    const WakeLockCtrl = {
        ...
        updateText() {
            ...
            const txtOn = config.wakeLockOn || "休眠開啟中";
            const txtOff = config.wakeLockOff || "休眠關閉中";

            if (WakeLockCtrl._isActiveMode) {
                textSpan.textContent = txtOff;
                lockBtn.classList.add('active'); // 100% 成功點亮為您截圖中的藍底黑字
            } else {
        ...
        request() {
            ...
            // 🌟 硬體強制鎖定：點火播放隱形無聲影片，欺騙 WebKit 核心，100% 強制 iPad 螢幕常亮不變暗！
            if (wakeVideo) {
                if (!wakeVideo.src) wakeVideo.src = dummyVideoSrc;
                wakeVideo.play().catch(() => { });
            }
            console.log("[WakeLock] 功能已啟用，隱形影片已在背景為 iPad 注入常亮護甲");
        },
        ...
        release() {
            ...
            // 🌟 硬體釋放：暫停隱形影片，將最高電力豁免權還給 iPad，重啟原生省電
            if (wakeVideo) {
                wakeVideo.pause();
            }
            console.log("[WakeLock] 功能已主動關閉，硬體常亮鎖已安全解鎖");
        }
    };
    ...
    lockBtn.onclick = () => {
        ...
        setTimeout(() => {
            ...
            // 在不透明度亮回 1、iOS 渲染核心塵埃落定的這一微秒，才由 updateText 執行外觀類別清洗！
    ...
    document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'visible') {
            if (WakeLockCtrl._isActiveMode) {
                WakeLockCtrl.request(); // 醒來瞬間，在背景悄悄為隱形影片重新點火續約，常亮絕不中斷
            } else {
                WakeLockCtrl.updateText();
            }
        }
