...
function renderPodcastList() {
    ...
    // 1. ⚡ 精準抓取 HTML 中帶有 ID 的 H1 標籤，百分之百讀取自 config
    ...
    // 2. 生成每一集的 HTML（無行內 margin 限制，完全由外部 CSS 控管）
    ...
    // 3. ⚡ 注入內容：將滾動控制層放到大框架裡面，並強制設定 100% 寬度防跑版
    container.innerHTML = `
    ...
    // 4. ⚡ 關鍵修改 3：針對 iOS 的「非同步阻斷」加入自動預熱防禦機制
    // 遍歷所有剛生成的播放器，綁定一次性事件。當使用者在 iOS 上第一次按下「播放」按鈕時，強迫它同步初始化
    container.querySelectorAll('audio').forEach(audio => {
        audio.addEventListener('play', function handleFirstPlay() {
            // 確保媒體在現有上下文被合法載入
            if (this.readyState === 0) {
                this.load();
                this.play().catch(() => { });
            }
            // 執行一次後即解除監聽，不影響後續的暫停與播放
            this.removeEventListener('play', handleFirstPlay);
        });
    });
