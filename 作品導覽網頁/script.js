...
/* ==========================================
 * 全站觸控最高防線：全自動多點觸控過濾網（100% 只放行單指操控、全平台 iOS 載入 0 延遲保活版）
 * ========================================== */
document.addEventListener('touchstart', function (event) { // 在網頁最外層最高節點註冊 touchstart 原生觸控開始事件，24小時全時段不間斷追蹤使用者的手指貼牆狀態
    if (event.touches.length > 1) { // 核心幾何判定：當手機晶片回報、偵測到玩家此時貼在螢幕上的實體手指數量大於 1 根（即雙指、三指等多點觸控手勢）時
        event.preventDefault(); // 終極正確阻斷：在微秒級時間內對 iOS 晶片發出最高硬體中斷指令，物理沒收此訊號的發射權限，全平台強行封鎖、禁用多點縮放與雙擊放大
    } // 結束多點觸控數量的條件檢查分支
}, { passive: false }); // 關鍵性能金鑰：將 passive 參數硬性宣告為 false，明確命令現代瀏覽器核心允許此監聽器具有最高優先權來執行 preventDefault() 阻斷，防止出現滾動黏滯延遲
                ...
                window.CustomPlayer = { // 在全域 window 物件上掛載一個名為 CustomPlayer 的獨立控制庫，用來非同步調度所有自定義播放器的圖片 src 切換與時間軸滑移
                    togglePlay(idx) { // 定義一個名為 togglePlay 的方法，用來開關特定集數的播音
                        ...
                        if (audio.paused) { // 條件判斷：若該集目前正處於暫停或未播放狀態
                            document.querySelectorAll('.player-control-btn').forEach((b, ai) => { if (ai !== idx) { const aud = document.getElementById(`native-audio-${ai}`); if (aud) aud.pause(); b.classList.remove('is-playing'); b.classList.add('is-paused'); const img = b.querySelector('.player-icon-img'); if (img) img.src = pImg; } }); // 排他機制：強迫其他所有集數暫停，並一對一將按鈕內部的圖片 src 全部換回常規播放去背圖檔
                            audio.play().then(() => { if (iconImg) iconImg.src = sImg; btn.classList.remove('is-paused'); btn.classList.add('is-playing'); }).catch(() => { if (iconImg) iconImg.src = pImg; }); // 終極正確修正：成功播音後直接將內部實體 img 標籤的 src 修改指派為暫停去背圖片路徑，並同步更新類別名稱
                        } else { // 否則（意即該集正在放音中）
                            audio.pause(); // 觸發暫停播音
                            if (iconImg) iconImg.src = pImg; // 將內部實體 img 標籤的 src 重新指派換回常規的播放去背圖檔路徑
                            btn.classList.remove('is-playing'); // 觸發暫停時移出播放狀態類別
