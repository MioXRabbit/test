...
/**
 * 獨立功能：實機演示影片渲染外掛（智慧型多語系鎖定防斷流版）
 * 仿照 Podcast 音檔精準替換機制：結構與影片完全鎖定，切換語言「只換文字」，音樂、進度 100% 不中斷
 */
function renderGameDemoVideos() {
    ...
    Object.entries(demoConfigs).forEach(([selector, videoData]) => {
        ...
        // ⚡ 1. 核心判斷：模仿音檔處理方式，檢查畫面上是不是「已經有」產生好的影片標籤（video）了？
        ...
        if (!scrollBoxVideo) {
            ...
            // 只有在第一次開啟網頁、畫面上完全沒東西時，才需要完整注入 HTML 結構
            const videoHtml = `
                <!-- 💡 特意加上 class="game-video-title" 標記，方便切換語言時精準鎖定 -->
                <h2 class="game-title game-video-title"></h2>
                ${videoData.src ? `
                    <div class="video-wrapper">
                        <video src="${videoData.src}" controls playsinline class="game-video"></video>
                    </div>
                ` : ''}
            `;
            ...
            // 我們不落地、不碰影片，直接精準抓出該區塊內現有的 .game-video-title 標記更新文字
            const titleEl = targetElement.querySelector('.game-video-title');
            if (titleEl) {
                // 將畫面的標題文字直接更換為 config 最新語系（中文或英文）的影片大標題
