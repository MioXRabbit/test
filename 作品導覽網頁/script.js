...
window.addEventListener('languageChanged', () => { // 監聽語系改變事件
    // 1. 💡 讓其他不需要複雜狀態保留的文字模組走全站渲染
    if (typeof runAllRenderModules === 'function') {
        // 如果 runAllRenderModules 會粗暴清空 Podcast 容器，
        // 請確保在該函式內部跳過（skip）Podcast 的 container 區塊！
        runAllRenderModules();
    }

    // 2. 💡 強制讓 Podcast 走我們寫好的精準去降級翻譯更新 (else 分支)
...
function renderChar(idx) {
    ...
    // 💡 關鍵新增：全域角色狀態狀態指針防線
    // ========================================================
    // 如果有傳入 idx（例如點擊頭像），就使用它並寫入全域；
    // 如果沒有傳入（例如切換語言），就優先讀取全域記錄，若都沒有則安全回退到預設的第一個角色 0
    ...
    // 💡 修正關鍵：改用經全域防線校正後的 currentCharIdx 去撈取當前語系包的角色資料 c
    // 這樣不論是誰、在何時觸發此函數，c 永遠都會是畫面上使用者正在觀看的那個角色！
        ...
        if (btnWrap) { // 檢查頭像按鈕容器是否存在
            config.chars.forEach((char, i) => { // 遍歷所有角色資料
                ...
                btn.onclick = () => { // 綁定頭像按鈕點擊事件
                    ...
                    // 📢 點擊頭像時，在切換前也同步更新全域指針，保持資料流绝对一致
                    window.CurrentCharIndex = i;
    ...
    const updateActiveButtonState = () => { // 定義更新頭像按鈕高亮狀態的函式
        ...
        if (allButtons && allButtons.length > 0) { // 檢查頭像按鈕是否存在且不為空
            allButtons.forEach((btn, i) => { // 遍歷所有頭像按鈕
                // 📢 修正關鍵：將高亮判斷同步對齊修正後的 currentCharIdx，確保翻譯後按鈕框不會位移框錯人
                if (i === currentCharIdx) {
            ...
            if (videoData.src) {
                videoHtml += `
                    <div class="video-wrapper">
                        <video controls preload="metadata" playsinline class="game-video" draggable="false"></video>
                    </div>
                `;
            }
