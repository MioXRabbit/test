       ...
       */
       ...
       document.addEventListener('DOMContentLoaded', () => {
           ...
           [renderNav, renderStaff, renderStaticContent, initMarquee].forEach(fn => fn());
   ...
   `).join('');
       ...
       function renderStaticContent() {
           const contents = {
               '#sec1 .content-box': `<h1>${config.introTitle}</h1><p class="p-text">${config.introText}</p>`,
               '#sec3 .content-box': `<h1>${config.gameIntroTitle}</h1><p class="p-text">${config.gameIntroText}</p>`
           };
       ...
       function renderPodcastList() {
           ...
           const listHtml = config.podcastList.map(item => `
            <div class="podcast-item">
                <img src="${item.cover}" style="width: 55px; height: 55px; border-radius: 5px; object-fit: cover; box-shadow: 0 4px 8px rgba(0,0,0,0.5);">
                <div style="flex: 1; overflow: hidden;">
                    <h4 style="color: #68BFFF; font-size: 14px; margin-bottom: 8px; text-align: left;">${item.title}</h4>
